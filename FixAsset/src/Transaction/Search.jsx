...
function Issue() {
  ...
  const Owner = (Id_owner) => {
    ...
    axios
      .post("http://10.17.100.183:3001/Id_owner", {
        owner_id: Id_owner,
      })
      .then((res) => {
        const data = res.data;
  ...
  const RequestType = async () => {
    try {
      const response = await axios.get(`http://10.17.100.183:3001/gettype`);
  ...
  const handleEdit = async (EditFam, index) => {
    ...
    try {
      const response = await axios.get(
        `http://10.17.100.183:3001/getEdit_routing?FamNo=${EditFam}`
      );
      const data = await response.data;
    ...
    if (Path === "SEARCH") {
      ...
      try {
        const rollNoSearch = await axios.get(
          `http://10.17.100.183:3001/getsearch?UserLogin=${UserLoginn}&FacCode=${selecteDatafac}&DeptCode=${selectdept}&FamNo=${FamNo}&FamTo=${FamTo}&Costcenter=${selectcost}&FixAsset=${FixAsset}&ReType=${selectReType}&ReDate=${Date}&ReDateTo=${DateTo}`
        );
      ...
      try {
        const rollNoSearch = await axios.get(
          `http://10.17.100.183:3001/getsearch2?UserLogin=${UserLoginn}&FacCode=${selecteDatafac}&DeptCode=${selectdept}&FamNo=${FamNo}&FamTo=${FamTo}&Costcenter=${selectcost}&FixAsset=${FixAsset}&ReType=${selectReType}&ReDate=${Date}&ReDateTo=${DateTo}`
        );
        const data = rollNoSearch.data;
        setCheckHead("visible");
        setdataSearch(data);
      ...
      axios
        .post("http://10.17.100.183:3001/searchFamMaster", {
          Fac: selecteDatafac,
  ...
  const Delete = async (item) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.post(
            `http://10.17.100.183:3001/delect_all_fam_transfer?famno=${item}`
          );
          await axios.post(
            `http://10.17.100.183:3001/delect_all_fam_details?famno=${item}`
          );
          await axios.post(
            `http://10.17.100.183:3001/delect_all_fam_header?famno=${item}`
          );
          await axios.post(
            `http://10.17.100.183:3001/delete_all_file?famno=${item}`
          );
