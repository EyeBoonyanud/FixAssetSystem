...
const SidebarMenu = ({ isOpen, onClose }) => {
  ...
  const Menu = async () => {
    try {
      const response = await axios.get(`http://10.17.74.201:5000/getmainmenu`);
  ...
  const subMenu = async () => {
    try {
      const response = await axios.get(
        `http://10.17.74.201:5000/getsubmenu?userlogin=${UserLogin}&role=${Role}`
      );
      const data = await response.data;
      let datasubmenu = [];
      let datasubmenuid = [];
  ...
  const handleButtonClick = (id) => {console.log(id,"YYYYYY")
    if (id === "Issue FAM") {
