...
function person_maintain_new({ isOpen, onClose, searchFunction }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const Level = async () => {
        try {
          const response = await axios.get(`http://10.17.100.183:3001/getlevel`);
        ...
        case "ok":
          if (PAGE_STATUS === "NEW") {
            if (
              ...
              try {
                const response = await axios.post(
                  `http://10.17.100.183:3001/ins_PERSON_MAINTAIN?FPM_factory=${selecteDatafac[0]}&FPM_level=${selecteDatalevel[0]}&FPM_cost_center=${selectcost[0]}&FPM_user_login=${User_Login}&FPM_email=${email}&FPM_status=${status}&FPM_create_by=${UserLoginn}&FPM_update_by=${UserLoginn}`
                );
            ...
            if (
              ...
              try {
                const response = await axios.post(
                  `http://10.17.100.183:3001/update_PERSON_MAINTAIN?FPM_factory=${selecteDatafac[0]}&FPM_level=${selecteDatalevel[0]}&FPM_cost_center=${selectcost[0]}&FPM_user_login=${User_Login}&FPM_email=${email}&FPM_status=${status}&FPM_update_by=${UserLoginn}`
                );
  ...
  const Check_Username_Email = async (user_login) => {
    ...
    try {
      const getDatalogin_show = await axios.get(
        `http://10.17.100.183:3001/getData_UserLogin_Person?User_Login=${user_login}`
      );
