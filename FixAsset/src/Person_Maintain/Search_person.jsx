...
function person_maintain() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const factoryPromise = axios.get(`http://10.17.100.183:3001/getfactory`);
        const levelPromise = axios.get(`http://10.17.100.183:3001/getlevel`);
        const costPromise = axios.get(`http://10.17.100.183:3001/getcost`);
    ...
    if (DATA_SEARCH_S_E !== null) {
      ...
      try {
        ...
        const rollNoSearch = await axios.get(
          `http://10.17.100.183:3001/Search_Person_Maintain?FPM_factory=${factoryValue}&FPM_level=${levelValue}&FPM_cost_center=${costValue}&FPM_user_login=${User_LoginValue}`
        );
  ...
  const handleOpenEdit = async (
    ...
    try {
      const getEdit_show = await axios.get(
        `http://10.17.100.183:3001/Search_Person_Maintain_Edit?FPM_factory=${factory}&FPM_level=${level}&FPM_cost_center=${cost_center}&FPM_user_login=${user_login}`
      );
      ...
      if (willDelete) {
        try {
          const delete_person_maintain = await axios.post(
            `http://10.17.100.183:3001/dlt_PERSON_MAINTAIN?FPM_factory_delete=${factory}&FPM_level_delete=${level}&FPM_cost_center_delete=${cost_center}&FPM_user_login_delete=${user_login}`
          );
