      ...
      const BOI_Project = async () => {
        try {
          const response = await axios.get(
            `http://10.17.100.183:3001/get_BOI_project`
          );
    ...
    if (DATA_SEARCH_S_E !== null) {
      ...
      try {
        ...
        const rollNoSearch = await axios.get(
          `http://10.17.100.183:3001/search_BOI_project?FBMC_factory=${factoryValue}&FBMC_cost_center=${costValue}&FBMC_BOI_project=${BOIValue}`
        );
  ...
  const handleOpenEdit = async (factory, cost_center, boi_project , index) => {
          ...
          try {
            const getEdit_show = await axios.get(
              `http://10.17.100.183:3001/Search_BOI_Maintain_Edit?FBMC_cost_center=${cost_center}&FBMC_BOI_Project=${boi_project}`
            );
      ...
      if (willDelete) {
        try {
          const delete_BOI_maintain = await axios.post(
            `http://10.17.100.183:3001/dlt_BOI_MAINTAIN?FBMC_cost_center_delete=${cost_center}&FBMC_BOI_Project_delete=${boi_project}`
          );
