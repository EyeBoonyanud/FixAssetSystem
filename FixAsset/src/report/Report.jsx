...
function Report() {
  ...
  const Type = () => {
    axios.post("http://10.17.74.202:5000/RequstType").then((res) => {
      const data = res.data;
      setTypeRequest(data);
    });
  };
  ...
  const Factory = () => {
    axios.get("http://10.17.74.202:5000/getfactory").then((res) => {
      const data = res.data;
      setgetFactory(data);
    });
  };
  ...
  const Owner = (Id_owner) => {
    ...
    axios.post("http://10.17.74.202:5000/Id_owner",{
      owner_id: Id_owner,
    }).then((res) => {
      const data = res.data;
      if(data.length>0){
  ...
  const Search = () => {
    if (selectFactory=== "" && selectCostCenter=== "" && Txt_FamNo_TO=== "" && Txt_ID_Owner=== "" && Txt_user=== "" && selectRequestType === "" && Txt_FamNo === "") {
      ...
      setCheckData("visible");
      axios
        .post("http://10.17.74.202:5000/FamDetailReport", {
          Fac:selectFactory,
          CC:selectCostCenter,
